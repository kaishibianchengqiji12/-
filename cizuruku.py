...
# }
#
# url = "http://192.168.1.10:8080/dict/cn/word/insert"
#
...
try:
    ...
    with open("shengsheng400.json", "r", encoding="utf-8") as f:
    ...
    for i in temp:
        if i['all_example']:
            print(i["word_group"])
            phoneticize = Pinyin()
            spell = phoneticize.get_pinyin(i["word_group"], ' ', tone_marks='marks')
            if i["chinese_voice"]:
            ...
            with open("cizu300.txt", "a", encoding="utf-8") as f:
                f.write(i["word_group"] + "\n")
            print(response.text)

        else:
            phoneticizet = Pinyin()
            spell = phoneticizet.get_pinyin(i["word_group"], ' ', tone_marks='marks')
            if i["chinese_voice"]:
                chinese_voice = data_voice(i["chinese_voice"])
            else:
                chinese_voice = ""
